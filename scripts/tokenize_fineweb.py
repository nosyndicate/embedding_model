...
def write_shard(
    ...
    if not isinstance(tokens, np.ndarray) or tokens.dtype != np.uint16:
        assert (tokens >= 0).all() and (
            tokens < 2**16
        ).all(), "Tokens must fit in uint16"
...
def main() -> None:
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path("data") / default_dir
    output_dir.mkdir(parents=True, exist_ok=True)

    print(f"FineWeb-Edu Pre-tokenization")
